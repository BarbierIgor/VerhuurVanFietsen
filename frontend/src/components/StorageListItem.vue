...
export default defineComponent({
    props: {
        bikeStation: {
            type: Object as () => BikeStation,
        },
    },
    methods: {
        handleFavorite(e: any, id: Number) {
...
<template>
    <li v-if="$props.bikeStation" :key="`${$props.bikeStation.id}`" class="flex h-28 justify-between mt-8 first:mt-0">
        ...
        <div class="flex flex-col justify-around flex-grow pl-4">
            <h1 class="text-dark-400">{{ `${$props.bikeStation.id} - ${$props.bikeStation.street}` }}</h1>
            <p class="text-dark-600">{{ $props.bikeStation.street }}</p>
            <div class="flex items-center">
                ...
                <p class="ml-2 text-dark-600">{{ `${$props.bikeStation.cost}km` }}</p>
            </div>
            <div class="flex items-center">
                ...
                <p class="ml-2 text-dark-600">{{ $props.bikeStation.available }}</p>
            </div>
        </div>
        <div class="favorite-checkbox">
            <input type="checkbox" hidden @change="handleFavorite($event, $props.bikeStation.id)" :id="`checkbox${$props.bikeStation.id}`" :checked="Boolean($props.bikeStation.favorite)">
            <label :for="`checkbox${$props.bikeStation.id}`">
                <svg class="h-full w-6" viewBox="0 0 24 21.208">
                    <g transform="translate(0 -27.5)">
                        <path class="transition-colors duration-200 fill-current text-dark-400" d="M22.01,29.506a6.8,6.8,0,0,0-9.631.005l-.371.371-.381-.381A6.809,6.809,0,0,0,0,34.314a6.751,6.751,0,0,0,2,4.813L11.276,48.4a1.041,1.041,0,0,0,.736.3,1.059,1.059,0,0,0,.736-.3L22,39.142a6.815,6.815,0,0,0,.01-9.637Z"/>
                        </g>
                </svg>
            </label>
        </div>
